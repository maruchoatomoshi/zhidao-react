  ...
  return (
    <div className="dur-dots">
      {Array.from({ length: max }).map((_, i) => (
        <span
          key={i}
          className={`dur-dot ${i < durability ? 'dur-dot-on' : 'dur-dot-off'}`}
        />
      ))}
    </div>
  );
  ...
  return (
    <div className={`genshin-card genshin-card-${info.stars}`}>
      ...
      <div className="genshin-card-info">
        ...
        <div className="genshin-card-stars">
          {'★'.repeat(info.stars)}
        </div>
...
const ImplantsPage = () => {
  ...
  // Авто-переключение вкладки в зависимости от темы
  useEffect(() => {
  ...
  return (
    <div className="page-container implants-page">
      ...
      <div className="page-hdr">
        ...
        <button className="lb-refresh-btn" onClick={tab === 'implants' ? loadImplants : loadCards} title="Обновить">
          <IconRefresh size={18} />
        </button>
      </div>
      ...
      <div className="subtabs">
        <button
          className={`subtab ${tab === 'implants' ? 'active' : ''}`}
          onClick={() => setTab('implants')}
        >
          🤝 ИМПЛАНТЫ
        </button>
        <button
          className={`subtab ${tab === 'cards' ? 'active' : ''}`}
      ...
      {loading ? (
        ...
        <div className="genshin-cards-list">
          {cards.length > 0 ? (
            cards.map((card, i) => (
              <GenshinCard key={card.id || i} card={card} />
            ))
          ) : (
            <div className="empty-state">
              <IconLock size={24} />
              <p>Карточек пока нет</p>
              <span className="hint">Делайте молитвы</span>
            </div>
          )}
        </div>
      )}
